...
export interface ExpertiseSlide {
  ...
  summary: string;
  bullets: string[];
  /** Ancre optionnelle posee sur le titre (rend un conteneur `scroll-mt-28`). */
  anchorId?: string;
}

interface ExpertiseHoverSliderProps {
  slides: ExpertiseSlide[];
  /** Hauteur minimale du bloc titre + resume du panneau de details. */
  summaryMinHeightClassName?: string;
}

function HoverSliderDetails({
  slides,
  summaryMinHeightClassName,
}: {
  slides: ExpertiseSlide[];
  summaryMinHeightClassName: string;
}) {
  ...
  return (
    <section className="mt-10 border border-slate-200 bg-slate-50/70 px-6 py-4">
      ...
      <div className={`mt-3 ${summaryMinHeightClassName}`}>
...
export function ExpertiseHoverSlider({
  slides,
  summaryMinHeightClassName = "min-h-[96px]",
}: ExpertiseHoverSliderProps) {
  return (
    <HoverSlider className="space-y-10">
      <div className="grid gap-8 lg:grid-cols-[1fr_1.1fr] lg:items-start">
        <div className="flex flex-col gap-4 text-lg font-semibold text-[#0A1A2F] md:text-xl">
          {slides.map((slide, index) => {
            ...
            if (!slide.anchorId) return trigger;

            return (
              <div key={slide.title} id={slide.anchorId} className="scroll-mt-28">
                {trigger}
              </div>
            );
          })}
      ...
      <HoverSliderDetails slides={slides} summaryMinHeightClassName={summaryMinHeightClassName} />
    </HoverSlider>
  );
}
